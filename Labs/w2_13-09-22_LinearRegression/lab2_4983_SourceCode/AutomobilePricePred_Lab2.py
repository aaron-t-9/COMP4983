...
data.drop(columns="normalized-losses", axis=1, inplace=True)
...
data_subset = data.loc[:, ['make', 'body-style', 'wheel-base', 'engine-size', 'horsepower',
                           'peak-rpm', 'highway-mpg', 'price']]
...
total_sum_sq = ((test_labels - price_pred)).sum()
res_sum_sq = ((test_labels - np.mean(test_labels))).sum()
CoD = 1 - (res_sum_sq/total_sum_sq)
print('Coefficient of Determination = ', CoD)

"""
Part 2: Linear Regression Model Feature Selection
"""

print("\n" + "="*10 + "   Part 2: Linear Regression Model Feature Selection   " + "="*10 + "\n")

features = ["make", "body-style", "wheel-base", "engine-size", "horsepower", "peak-rpm", "highway-mpg"]

for feature in features:
    print(f"---------- Feature: {feature.title()} ----------")
    data_subset = data.loc[:, [feature, "price"]]

    if feature in ['wheel-base', 'engine-size', 'horsepower', 'peak-rpm', 'highway-mpg', 'price']:
        data_subset[[feature, 'price']] = data_subset[[feature, 'price']].astype(float)

    if feature in ["make", "body-style"]:
        data_subset = pd.get_dummies(data_subset, columns=[feature], prefix=[feature])
...
print("\033[1m" +
      "Engine-Size has the lowest MAE out of all of the features with an MAE of 2114.236" +
      "\033[0m" +
      "\n\n")
